...
guard 'jasmine-node', :jasmine_node_bin => "#{ENV['HOME']}/node_modules/jasmine-node/bin/jasmine-node" do
  watch(%r{^spec(.*/)?(.+)_spec\.(js\.coffee|js|coffee)$})  { |m| "spec#{m[1]}" }
  watch(%r{^lib(.*/)(.+)\.(js\.coffee|js|coffee)$})        { |m| "spec#{m[1]}" }
